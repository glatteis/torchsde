...
def test_rename_methods_logqp(device):
    ...
    ts = torch.linspace(t0, t1, steps=T, device=device)
    ans = torchsde.sdeint(
        sde, y0, ts, dt=dt, names={"drift": "forward", "prior_drift": "w"}, logqp=True
    )
...
@pytest.mark.parametrize(
    "sde_type,method", [("ito", "euler"), ("stratonovich", "midpoint")]
)
def test_specialised_functions(sde_type, method):
    ...
    for sde in (
        ...
        g_prod,
        f_and_g_prod,
        f_and_g_with_g_prod1,
        f_and_g_with_g_prod2,
    ):
...
@pytest.mark.parametrize(
    "use_bm,levy_area_approximation", _use_bm__levy_area_approximation()
)
...
@pytest.mark.parametrize("device", devices)
def test_sdeint_run_shape_method(
    sde_cls, use_bm, levy_area_approximation, sde_type, method, adaptive, logqp, device
):
    """Tests that sdeint:
...
@pytest.mark.parametrize(
    "sde_cls",
    [problems.BasicSDE1, problems.BasicSDE2, problems.BasicSDE3, problems.BasicSDE4],
)
...
@pytest.mark.parametrize(
    "sde_cls",
    [
        problems.NeuralDiagonal,
        problems.NeuralScalar,
        problems.NeuralAdditive,
        problems.NeuralGeneral,
    ],
)
def test_reversibility(sde_cls):
    ...
    bm = torchsde.BrownianInterval(
        t0=ts[0], t1=ts[-1], size=(batch_size, brownian_size)
    )
    ys, (f, g, z) = torchsde.sdeint(
        sde, y0, ts, bm=bm, method="reversible_heun", dt=dt, extra=True
    )
    backward_ts = -ts.flip(0)
    backward_ys = torchsde.sdeint(
        minus_sde,
        ys[-1],
        backward_ts,
        bm=torchsde.ReverseBrownian(bm),
        method="reversible_heun",
        dt=dt,
        extra_solver_state=(-f, -g, z),
    )
