...
Data = namedtuple(
    "Data", ["ts_", "ts_ext_", "ts_vis_", "ts", "ts_ext", "ts_vis", "ys", "ys_"]
)
...
class LatentSDE(torchsde.SDEIto):
    ...
    def sample_p(self, ts, batch_size, eps=None, bm=None):
        ...
        return sdeint_fn(
            self, y0, ts, bm=bm, method="srk", dt=args.dt, names={"drift": "h"}
        )
...
def make_segmented_cosine_data():
    ...
    ts_ext_ = np.array([0.0] + list(ts_) + [2.0])
    ts_vis_ = np.linspace(0.0, 2.0, 300)
    ys_ = np.cos(ts_ * (2.0 * math.pi))[:, None]
...
def make_irregular_sine_data():
    ...
    ts_ext_ = np.array([0.0] + list(ts_) + [2.0])
    ts_vis_ = np.linspace(0.0, 2.0, 300)
    ys_ = np.sin(ts_ * (2.0 * math.pi))[:, None] * 0.8
...
def main():
    ...
    if args.show_prior:
        with torch.no_grad():
            zs = model.sample_p(
                ts=ts_vis, batch_size=vis_batch_size, eps=eps, bm=bm
            ).squeeze()
            ...
            for alpha, percentile in zip(alphas, percentiles):
                idx = int((1 - percentile) / 2.0 * vis_batch_size)
                zs_bot_ = zs_[:, idx]
                zs_top_ = zs_[:, -idx]
                plt.fill_between(
                    ts_vis_, zs_bot_, zs_top_, alpha=alpha, color=fill_color
                )
    ...
    for global_step in tqdm.tqdm(range(args.train_iters)):
        ...
        zs = zs[
            1:-1
        ]  # Drop first and last which are only used to penalize out-of-data region and spread uncertainty.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save-ckpt", type=str2bool, default=False, const=True, nargs="?"
    )

    parser.add_argument(
        "--data",
        type=str,
        default="segmented_cosine",
        choices=["segmented_cosine", "irregular_sine"],
    )
    parser.add_argument(
        "--kl-anneal-iters",
        type=int,
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of iterations before pausing.",
    )
    parser.add_argument(
        "--batch-size", type=int, default=512, help="Batch size for training."
    )
    parser.add_argument(
        "--likelihood", type=str, choices=["normal", "laplace"], default="laplace"
    )
    parser.add_argument(
        "--scale",
        type=float,
        default=0.05,
        help="Scale parameter of Normal and Laplace.",
    )

    parser.add_argument(
        "--adjoint", type=str2bool, default=False, const=True, nargs="?"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--show-prior", type=str2bool, default=True, const=True, nargs="?"
    )
    parser.add_argument(
        "--show-samples", type=str2bool, default=True, const=True, nargs="?"
    )
    parser.add_argument(
        "--show-percentiles", type=str2bool, default=True, const=True, nargs="?"
    )
    parser.add_argument(
        "--show-arrows", type=str2bool, default=True, const=True, nargs="?"
    )
    parser.add_argument(
        "--show-mean", type=str2bool, default=False, const=True, nargs="?"
    )
    parser.add_argument(
        "--hide-ticks", type=str2bool, default=False, const=True, nargs="?"
    )
