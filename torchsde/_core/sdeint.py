...
def sdeint(
    ...
    logqp: bool = False,
    logqp_noise_penalty: bool = False,
    extra: bool = False,
    ...
    **unused_kwargs,
    ...
    Args:
        ...
        logqp_noise_penalty (bool, optional): If `True`, also return the
        log-ratio penalty and noise penalty.
    ...
    sde, y0, ts, bm, method, options = check_contract(
        sde, y0, ts, bm, method, adaptive, options, names, logqp, logqp_noise_penalty
    )
    misc.assert_no_grad(
        ["ts", "dt", "rtol", "atol", "dt_min"], [ts, dt, rtol, atol, dt_min]
    )
...
def check_contract(
    ...
    if sde.noise_type not in NOISE_TYPES:
        raise ValueError(
            f"Expected noise type in {NOISE_TYPES}, but found {sde.noise_type}."
        )
    ...
    if y0.dim() != 2:
        raise ValueError(
            "`y0` must be a 2-dimensional tensor of shape (batch, channels)."
        )

    # --- Backwards compatibility: v0.1.1. ---
    if logqp_noise_penalty:
        sde = base_sde.SDELogqpNoisePenalty(sde)
        y0 = torch.cat((y0, y0.new_zeros(size=(y0.size(0), 2))), dim=1)
    elif logqp:
    ...
    if not torch.is_tensor(ts):
        if not isinstance(ts, (tuple, list)) or not all(
            isinstance(t, (float, int)) for t in ts
        ):
            raise ValueError(
    ...
    def _check_2d(name, shape):
        if len(shape) != 2:
            raise ValueError(
                f"{name} must be of shape (batch, state_channels), but got {shape}."
            )
    ...
    def _check_2d_or_3d(name, shape):
        if sde.noise_type == NOISE_TYPES.diagonal:
            ...
            if len(shape) != 3:
                raise ValueError(
                    f"{name} must be of shape (batch, state_channels, noise_channels), but got {shape}."
                )
            batch_sizes.append(shape[0])
            state_sizes.append(shape[1])
    ...
    if hasattr(sde, "g_prod"):
        ...
        if len(noise_sizes) == 0:
            raise ValueError(
                "Cannot infer noise size (i.e. number of Brownian motion channels). Either pass `bm` "
                "explicitly, or specify one of the `g`, `f_and_g` functions.`"
            )
        v = torch.randn(
            batch_sizes[0], noise_sizes[0], dtype=y0.dtype, device=y0.device
        )
    ...
    if not has_f:
        raise ValueError(
            "sde must define at least one of `f`, `f_and_g`, or `f_and_g_prod`. (Or possibly more "
            "depending on the method chosen.)"
        )
    if not has_g:
        raise ValueError(
            "sde must define at least one of `g`, `f_and_g`, `g_prod` or `f_and_g_prod`. (Or possibly "
    ...
    if sde.noise_type == NOISE_TYPES.scalar:
        if noise_sizes[0] != 1:
            raise ValueError(
                f"Scalar noise must have only one channel; the diffusion has {noise_sizes[0]} noise "
                f"channels."
            )
    ...
    if bm is None:
        ...
        bm = BrownianInterval(
            t0=ts[0],
            t1=ts[-1],
            size=(batch_sizes[0], noise_sizes[0]),
            dtype=y0.dtype,
            device=y0.device,
    ...
    if adaptive and method == METHODS.euler and sde.noise_type != NOISE_TYPES.additive:
        warnings.warn(
            "Numerical solution is not guaranteed to converge to the correct solution when using adaptive "
