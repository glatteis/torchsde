...
class _SdeintAdjointMethod(torch.autograd.Function):
    ...
    def forward(
        ...
        *extras_and_adjoint_params,
        ...
        if (
            method == METHODS.reversible_heun
            and adjoint_method == METHODS.adjoint_reversible_heun
        ):
    ...
    def backward(ctx, grad_ys, *grad_extra_solver_state):  # noqa
        ...
        solver_fn = methods.select(
            method=ctx.adjoint_method, sde_type=adjoint_sde.sde_type
        )
        solver = solver_fn(
            sde=adjoint_sde,
            bm=reverse_bm,
        ...
        for i in range(ys.size(0) - 1, 0, -1):
            (_, aug_state), *extra_solver_state = _SdeintAdjointMethod.apply(
                adjoint_sde,
...
def sdeint_adjoint(
    ...
    **unused_kwargs,
    ...
    if adjoint_params is None and not isinstance(sde, nn.Module):
        raise ValueError(
            "`sde` must be an instance of nn.Module to specify the adjoint parameters; alternatively they "
    ...
    sde, y0, ts, bm, method, options = sdeint.check_contract(
    ...
    adjoint_params = (
        tuple(sde.parameters()) if adjoint_params is None else tuple(adjoint_params)
    )
    ...
    if method == METHODS.reversible_heun:
        if adjoint_method != METHODS.adjoint_reversible_heun:
            warnings.warn(
                f"method={repr(method)}, but adjoint_method!={repr(METHODS.adjoint_reversible_heun)}."
            )
        if adaptive or adjoint_adaptive:
            warnings.warn(
                f"A limitation of the current method={repr(method)} implementation is "
            ...
            if not torch.allclose(num_steps, num_steps.round()):
                warnings.warn(
                    f"The spacing between time points `ts` is not an integer multiple of the time step `dt`. "
...
def _select_default_adjoint_method(
    sde: base_sde.ForwardSDE, method: str, adjoint_method: Optional[str]
) -> str:
