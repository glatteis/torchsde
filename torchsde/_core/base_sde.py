...
class BaseSDE(abc.ABC, nn.Module):
    ...
    def __init__(self, noise_type, sde_type):
        ...
        if noise_type not in NOISE_TYPES:
            raise ValueError(
                f"Expected noise type in {NOISE_TYPES}, but found {noise_type}"
            )
...
class ForwardSDE(BaseSDE):
    def __init__(self, sde, fast_dg_ga_jvp_column_sum=False):
        ...
        self.prod = {NOISE_TYPES.diagonal: self.prod_diagonal}.get(
            sde.noise_type, self.prod_default
        )
        ...
        self.dg_ga_jvp_column_sum = {
            NOISE_TYPES.general: (
                self.dg_ga_jvp_column_sum_v2
                if fast_dg_ga_jvp_column_sum
                else self.dg_ga_jvp_column_sum_v1
            )
    ...
    def f_default(self, t, y):
        raise RuntimeError(
            "Method `f` has not been provided, but is required for this method."
        )
    ...
    def g_default(self, t, y):
        raise RuntimeError(
            "Method `g` has not been provided, but is required for this method."
        )
...
class RenameMethodsSDE(BaseSDE):
    def __init__(
        ...
        diffusion="g",
        prior_drift="h",
        diffusion_prod="g_prod",
        drift_and_diffusion="f_and_g",
        drift_and_diffusion_prod="f_and_g_prod",
    ):
        super(RenameMethodsSDE, self).__init__(
            noise_type=sde.noise_type, sde_type=sde.sde_type
        )
        self._base_sde = sde
        for name, value in zip(
            ("f", "g", "h", "g_prod", "f_and_g", "f_and_g_prod"),
            (
                drift,
                diffusion,
                prior_drift,
...
class SDEStratonovich(BaseSDE):
    def __init__(self, noise_type):
        super(SDEStratonovich, self).__init__(
            noise_type=noise_type, sde_type=SDE_TYPES.stratonovich
        )
    ...
    def __init__(self, sde):
        ...
        try:
            ...
            raise AttributeError(
                "If using logqp then drift, diffusion and prior drift must all be specified."
            ) from e
...
class SDELogqpNoisePenalty(BaseSDE):
    def __init__(self, sde):
        super(SDELogqpNoisePenalty, self).__init__(
            noise_type=sde.noise_type, sde_type=sde.sde_type
        )
        self._base_sde = sde
        ...
        if sde.noise_type == NOISE_TYPES.diagonal:
            ...
            # I don't want to accidentally train a non-diagonal SDE
            raise AttributeError(
                "SDELogqpNoisePenalty not defined for non-diagonal SDEs."
            )
    ...
    def f_diagonal(self, t, y: Tensor):
        ...
        u = misc.stable_division(f - h, g)
        f_logqp = 0.5 * (u**2).sum(dim=1, keepdim=True)
        noise_penalty = g.sum(dim=1, keepdim=True)
