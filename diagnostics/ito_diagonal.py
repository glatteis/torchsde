...
def main():
    ...
    t0, t1, steps, dt = 0.0, 2.0, 10, 1e-1
    ...
    bm = BrownianInterval(
        t0=t0,
        t1=t1,
        size=(large_batch_size, d),
        dtype=y0.dtype,
        device=device,
        levy_area_approximation=LEVY_AREA_APPROXIMATIONS.space_time,
    )
    inspection.inspect_orders(
        y0, t0, t1, dts, sde, bm, img_dir, methods, options, labels
    )
