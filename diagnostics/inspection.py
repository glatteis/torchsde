...
def inspect_orders(
    ...
    strong_order_slopes = [
        utils.linregress_slope(utils.log(dts), 0.5 * utils.log(mses_for_method))
        ...
        plots=[
            {
                "x": dts,
                "y": mses_for_method,
                "label": f"{label}(k={slope:.4f})",
                "marker": "x",
            }
            for mses_for_method, label, slope in zip(
                zip(*mses), labels, strong_order_slopes
            )
        ],
    ...
    utils.swiss_knife_plotter(
        ...
        plots=[
            ...
            for mres_for_method, label, slope in zip(
                zip(*maes), labels, weak_order_slopes
            )
        ],
        options={"xscale": "log", "yscale": "log", "cycle_line_style": True},
    )
